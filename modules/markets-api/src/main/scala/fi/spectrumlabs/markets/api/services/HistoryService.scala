...
object HistoryService {
  ...
  final private class Live[F[_]: Monad](ordersRepository: OrdersRepository[F]) extends HistoryService[F] {
    override def getUserHistory(query: HistoryApiQuery, paging: Paging, window: TimeWindow): F[OrderHistoryResponse] =
      query.userPkhs.flatTraverse(ordersRepository.getUserOrdersByPkh).map { orders =>
        val finalOrders = orders.flatMap(UserOrderInfo.fromDbOrder)
