...
object TxTrackerProgram {
  ...
  def create[
    ...
    logs: Logs[I, F]
  ): I[TxTrackerProgram[S]] =
    logs.forService[TxTrackerProgram[S]].map(implicit __ => new Impl[S, F, C](producer, config))
        ...
        explorer
          ...
          .evalMap { batch =>
            info"Received batch of ${batch.size} elems."
              ...
              .flatMap { _ =>
                cache.setLastTxOffset(batch.size + offset)
              }
              .flatMap { _ =>
